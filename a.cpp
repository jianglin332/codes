    ...
    {
        return point(a.pnt + a.vct / 2);
    }
    edge bisect(point a, point m, point b)
    {
        vect c = (a - m).to1();
        vect d = (b - m).to1();
        return edge(m, c + d, 1);
    }
    point proj(point a, edge b)
    {
        return intersect(edge(a, b.vct.rotate(pi / 2), 1), b, 0);
    }
    double dist(point a, point b)
    {
        return (a - b).r;
    }
    twopoint ccintersect(circle a, circle b)
    {
        ...
        if (l > c + d || l < abs(c - d))
            return twopoint();
        if (l == c + d)
            return twopoint(a.o + point(a.r, e.vct.theta, POL));
        double th;
        th = atan2(sqrt(2 * c * c * l * l + 2 * c * c * d * d +
                                2 * l * l * d * d - c * c * c * c - l * l * l * l -
                                d * d * d * d), c * c + l * l - d * d);
        point f = a.o + point(a.r, e.vct.theta + th, POL);
        point g = a.o + point(a.r, e.vct.theta - th, POL);
        return twopoint(f, g);
    }
    int main()
    {
